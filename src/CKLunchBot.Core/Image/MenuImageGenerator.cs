...
namespace CKLunchBot.Core.Image
{
    public class MenuImageGenerator
    {
        public static byte[] GenerateImage(List<MenuItem> menus)
        {
            ...
            try
            {
                using var generator = new ImageGenerator(Path.Combine("assets", "images", "menu_template.png"));
                ...
                foreach (var weekMenu in menus)
                {
                    ...
                    var contentText = todaysMenu is null ? "(오늘 제공한 메뉴 없음)" : CreateMenuString(todaysMenu);
                    ...
                    {
                        case "다반":
                            drawPosition.x = 60.0f;
                            break;

                        case "난카츠난우동":
                            drawPosition.x = 353.0f;
                            break;

                        case "탕&찌개차림":
                            drawPosition.x = 647.0f;
                            break;

                        case "육해밥":
                            drawPosition.x = 940.0f;
                            break;
                    ...
                    if (doDrawMenu)
                    {
                        generator.DrawText(drawPosition, generator.Fonts["content"], black, titleText);
                    }
                }
        ...
        private static string CreateMenuString(string[] menu) => ":: " + string.Join("\n:: ", menu);
    }
}